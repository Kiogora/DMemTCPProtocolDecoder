class protocol{
    ...
    async processMessages(args) {
        ...
        /*Continue processing data, persisting it within the object*/
        let arrShapedData = []
        if(response.arrBufAllData?.length > 0){
            response.arrBufAllData.map((async allData => {
                ...
                shapedData.time = Math.floor(processTime(shapedData.time)/1000)
                ...
                allData.arrFields.map(async field => {
                    switch (field.fId) {
                        case (0): 
                            ...
                            shapedData.gpsUTCDateTime = Math.floor(processTime(shapedData.gpsUTCDateTime)/1000)
            ...
            this.dataToCommit.arrShapedData = arrShapedData;
        }
        return response;
    }
}
